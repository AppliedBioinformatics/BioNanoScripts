...
if __name__=="__main__":
    ...
    parser.add_argument('-e', dest='enzymes', nargs='*', help='enzymes to digest the NGS sequences [BspQI|BbvCI|BsmI|BsrDI|bseCI|BssSI|HindIII]. Can be one or multiple.', type = str) # users can modify this to add new enzymes using same format
    ...
    if None not in [args.fasta, args.enzymes, args.output]:
        for enz in args.enzymes:
            ...
            run=cmapMaker(args.fasta, args.enzymes, args.length, args.site, args.threads, args.output)
            run.check_fa()
            run.check_path()
            run.checkPathW()
            run.digestion()
